...
$$\tau_i(\phi) = \begin{cases} \dfrac{d \cdot f_i^{\,k(\phi)}}{v_0(\phi)} & f_i \leq f_\text{max}(\phi) \\ \dfrac{\bigl(d + \delta(\phi)\bigr) \cdot \alpha}{v_0(\phi)} & f_i > f_\text{max}(\phi) \end{cases}$$
